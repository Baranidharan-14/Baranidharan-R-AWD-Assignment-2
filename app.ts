...
let recentPlayed: [Song?, Song?, Song?, Song?, Song?] = [];


function renderSongs(list: Song[]) {
...
function updateRecent(song: Song) {
  recentPlayed.unshift(song);
  recentPlayed = recentPlayed.slice(0, 5) as any;
...
function filterByGenre() {
  const genre = (document.getElementById("genreFilter") as HTMLSelectElement).value;
...
function filterByArtist() {
  const artist = (document.getElementById("artistFilter") as HTMLSelectElement).value;
...
document.getElementById("searchBar")!.addEventListener("input", (e: any) => {
